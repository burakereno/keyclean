...
final class UpdateChecker: ObservableObject {
    ...
    private static let repo = "keyclean"
    private static let assetName = "KeyClean.dmg"
    private static let productionBundleIdentifier = "dev.local.KeyClean"
    ...
    var updateAvailable: Bool {
        guard isProductionBuild, let latestVersion, downloadURL != nil else { return false }
    ...
    func checkForUpdates(force: Bool = false) async {
        ...
        request.setValue("KeyClean-UpdateChecker", forHTTPHeaderField: "User-Agent")
    ...
    func downloadAndInstall() {
        ...
        let task = URLSession.shared.downloadTask(with: downloadURL) { [weak self] tmpURL, _, error in
            Task { @MainActor in
                ...
                let fileName = "KeyClean-\(self.latestVersion ?? "latest")-\(UUID().uuidString).dmg"
    ...
    private func installUpdate(dmgURL: URL, expectedVersion: String) {
        ...
        let targetBundle = currentBundle.path.hasPrefix("/Applications/")
            ? currentBundle
            : URL(fileURLWithPath: "/Applications/KeyClean.app")

        let scriptURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("keyclean-install-\(UUID().uuidString).sh")
        let logURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("keyclean-install.log")
        let mountPointURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("keyclean-mount-\(UUID().uuidString)")
        ...
        STAGED_APP="$TARGET_PARENT/.KeyClean.app.update.$$"
        BACKUP_APP="$TARGET_PARENT/.KeyClean.app.previous.$$"
        MOUNT_ATTACHED=0
        ...
        SRC="$MOUNT_POINT/KeyClean.app"
        if [ ! -d "$SRC" ]; then
            fail "source app not found at $SRC"
        fi

        SRC_INFO="$SRC/Contents/Info.plist"
        SRC_EXEC="$SRC/Contents/MacOS/KeyClean"
