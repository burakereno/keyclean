...
final class UpdateChecker: ObservableObject {
    ...
    private static let manifestName = "KeyClean.dmg.update.json"
    private static let productionBundleIdentifier = "dev.local.KeyClean"
    private static let appBundleName = "KeyClean.app"
    private static let executableName = "KeyClean"
        ...
        let task = URLSession.shared.downloadTask(with: downloadURL) { [weak self] tmpURL, response, error in
            Task { @MainActor in
                ...
                let workDirectory = FileManager.default.temporaryDirectory
                    .appendingPathComponent("keyclean-update-\(UUID().uuidString)", isDirectory: true)
